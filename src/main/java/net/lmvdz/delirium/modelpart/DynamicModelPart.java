...
/**
 * Each DynamicModelPart has DynamicCuboid(s)
 * Each DynamicCuboid has a DynamicPart[] (one DynamicPart for each DYNAMIC_ENUM)
 * Each DynamicCuboid can calculate new offsets for each DynamicPart based on deltaTick or tick
 * Each DynamicCuboid can shift it's texture UV array
 */
public class DynamicModelPart extends ModelPart {
    ...
    public static final boolean[] DEFAULT_STATE = new boolean[]{true, true, true, true, true, true, true, true};
    ...
    public static final float[] DEFAULT_APPLY_RANDOM_MAX = new float[] {.005F, .005F, .005F, .005F, .005F, .005F, .005F, .005F};
    public static final float[] DEFAULT_APPLY_RANDOM_MIN = new float[] {0F, 0F, 0F, 0F, 0F, 0F, 0F, 0F};
    ...
    private float textureWidth; // texture width
    private float textureHeight; // texture height
    private int textureOffsetU; // texture offset u
    private int textureOffsetV; // texture offset v
    private ObjectList<DynamicModelPart.DynamicCuboid> cuboids; // array of cuboids attached to this DynamicModelPart
    private ObjectList<DynamicModelPart> children; // array of children DynamicModelPart(s) attached to this DynamicModelPart
    public static final DynamicModelPart EMPTY = new DynamicModelPart(0, 0, 0, 0); // EMPTY DynamicModelPart
     ...
     * @param seeds ObjectList<DynamicPart[]>, List of DyanmicPart arrays corresponding to each DynamicCuboid
     * @return DynamicModelPart instance
     */
    ...
    /**
     * Rotates the model based on the rotation given.
     * Adds cuboids to the current list of cuboids
    ...
    /**
     * Creates a new DynamicModelPart given basic ModelPart data along with seeds, spriteId, and layerFactory
     * @param dynamicModel
     * @param x
     * @param y
     * @param z
    ...
    /**
     * Creates a new DynamicModelPart with basic ModelPart data along with booleans for UV shifting and updating dynamics
     * @param dynamicModel
     * @param x
     * @param y
     * @param z
     ...
     */
    public DynamicModelPart(DynamicModel dynamicModel, float[] x, float[] y, float[] z, int[] sizeX, int[] sizeY, int[] sizeZ, float[] extra, int[] u, int[] v, float[] rotation, ObjectList<DynamicModelPart.DynamicPart[]> seeds, SpriteIdentifier spriteId, Function<Identifier, RenderLayer> layerFactory, boolean UV_SHIFT_APPLY_SYNC, boolean UV_SHIFTABLE, int UV_SHIFT_AMOUNT, boolean UV_SHIFT_EVERY_X_TICK, boolean UV_SHIFT_EVERY_TICK, int UV_SHIFT_EVERY_X_TICKS, boolean UV_SHIFT_EVERY_DELTA_TICK, boolean UV_SHIFT_EVERY_X_DELTA_TICK, int UV_SHIFT_EVERY_X_DELTA_TICKS, boolean UPDATE_DYNAMICS_EVERY_TICK, boolean UPDATE_DYNAMICS_EVERY_X_TICK, int UPDATE_DYNAMICS_EVERY_X_TICKS, boolean UPDATE_DYNAMICS_EVERY_DELTA_TICK, boolean UPDATE_DYNAMICS_EVERY_X_DELTA_TICK, int UPDATE_DYNAMICS_EVERY_X_DELTA_TICKS) {
        super(dynamicModel);
        this.cuboids = new ObjectArrayList<DynamicModelPart.DynamicCuboid>();
        this.children = new ObjectArrayList<DynamicModelPart>();
        this.with(UV_SHIFT_APPLY_SYNC, UV_SHIFTABLE, UV_SHIFT_AMOUNT, UV_SHIFT_EVERY_X_TICK, UV_SHIFT_EVERY_TICK, UV_SHIFT_EVERY_X_TICKS, UV_SHIFT_EVERY_DELTA_TICK, UV_SHIFT_EVERY_X_DELTA_TICK, UV_SHIFT_EVERY_X_DELTA_TICKS, UPDATE_DYNAMICS_EVERY_TICK, UPDATE_DYNAMICS_EVERY_X_TICK, UPDATE_DYNAMICS_EVERY_X_TICKS, UPDATE_DYNAMICS_EVERY_DELTA_TICK, UPDATE_DYNAMICS_EVERY_X_DELTA_TICK, UPDATE_DYNAMICS_EVERY_X_DELTA_TICKS)
            .setX(x)
            .setY(y)
            .setZ(z)
            .setSizeX(sizeX)
     ...
     * @param UV_SHIFT_APPLY_SYNC boolean, Should the UV shift be applied when dynamics are applied?
     * @param UV_SHIFTABLE boolean, Is the UV even shiftable?
     * @param UV_SHIFT_AMOUNT int, number of indexes to shift the UV array by
     * @param UV_SHIFT_EVERY_X_TICK boolean, Should the UV shift every X amount of ticks?
     ...
     * @param UV_SHIFT_EVERY_X_DELTA_TICK boolean, Should the UV shift evert X amount of deltaTicks?
     * @param UV_SHIFT_EVERY_X_DELTA_TICKS int, number of deltaTicks between each UV shift
     * @param UPDATE_DYNAMICS_EVERY_TICK boolean, should DynamicPart applyDynamics every tick?
     * @param UPDATE_DYNAMICS_EVERY_X_TICK boolean, should DynamicPart applyDynamics every x amount of ticks?
     * @param UPDATE_DYNAMICS_EVERY_X_TICKS int, number of ticks between each applyDynamics
     * @param UPDATE_DYNAMICS_EVERY_DELTA_TICK boolean, should DynamicPart applyDynamics every delta tick?
     * @param UPDATE_DYNAMICS_EVERY_X_DELTA_TICK boolean, should DynamicPart applyDynamics every x amount of delta ticks?
     * @param UPDATE_DYNAMICS_EVERY_X_DELTA_TICKS int, number of deltaTicks between each applyDynamics
     * @return DynamicModelPart instance
     */
    public DynamicModelPart with(boolean UV_SHIFT_APPLY_SYNC, boolean UV_SHIFTABLE, int UV_SHIFT_AMOUNT, boolean UV_SHIFT_EVERY_X_TICK, boolean UV_SHIFT_EVERY_TICK, int UV_SHIFT_EVERY_X_TICKS, boolean UV_SHIFT_EVERY_DELTA_TICK, boolean UV_SHIFT_EVERY_X_DELTA_TICK, int UV_SHIFT_EVERY_X_DELTA_TICKS, boolean UPDATE_DYNAMICS_EVERY_TICK, boolean UPDATE_DYNAMICS_EVERY_X_TICK, int UPDATE_DYNAMICS_EVERY_X_TICKS, boolean UPDATE_DYNAMICS_EVERY_DELTA_TICK, boolean UPDATE_DYNAMICS_EVERY_X_DELTA_TICK, int UPDATE_DYNAMICS_EVERY_X_DELTA_TICKS) {
     ...
     * @param children ObjectList<DynamicModelPart>, array of DynamicModelPart(s) to add to the children of this instance of DynamicModelPart
     * @return DynamicModelPart instance
     */
    ...
    public class DynamicPart {
        ...
        public DynamicPart(DYNAMIC_ENUM dynamic) {
            this.dynamic(dynamic, true, DEFAULT_MIN.clone()[dynamic.ordinal()], DEFAULT_MAX.clone()[dynamic.ordinal()], 0F, (float)Math.random(), DEFAULT_APPLY_RANDOM_MAX.clone()[dynamic.ordinal()], DEFAULT_APPLY_RANDOM_MIN.clone()[dynamic.ordinal()], DEFAULT_APPLY_RANDOM_MULTIPLIER.clone()[dynamic.ordinal()]).apply(true);
        }
        /**
        ...
        public DynamicPart(DYNAMIC_ENUM dynamic, boolean state) {
            this.dynamic(dynamic, state, DEFAULT_MIN.clone()[dynamic.ordinal()], DEFAULT_MAX.clone()[dynamic.ordinal()], 0F, (float)Math.random(), DEFAULT_APPLY_RANDOM_MAX.clone()[dynamic.ordinal()], DEFAULT_APPLY_RANDOM_MIN.clone()[dynamic.ordinal()], DEFAULT_APPLY_RANDOM_MULTIPLIER.clone()[dynamic.ordinal()]).apply(true);
        }
        /**
        ...
        public DynamicPart apply(boolean shouldApplyDynamics) {
            if (this.state) {
                // System.out.println(this.dynamic.toString() + " " + round(this.value, 3) + " " + round(this.newValue, 3));
                ...
                double lerp1 = Interpolation.LinearInterpolate(this.value, this.newValue, this.lerpPercent);
                double lerp2 = Interpolation.LinearInterpolate(lerp1, this.newValue, this.lerpPercent);
                // System.out.println(this.value +  " " + lerp1 + " " + lerp2 + " " + this.newValue);
                this.value = (float)Interpolation.CubicInterpolate(this.value, lerp1, lerp2, this.newValue, this.lerpPercent);
    ...
    public class DynamicCuboid extends Cuboid {
        ...
        private DynamicPart[] parts; // DynamicParts attached to this cuboid
        private DynamicModelPart.DynamicQuad[] sides; // each face of the cuboid
        private float x; // x coord
        private float y; // y coord
        private float z; // z coord
        private float sizeX; // size in x direction
        private float sizeY; // size in y direction
        private float sizeZ; // size in z direction
        private float extraX; // extra X
        private float extraY; // extra Y
        private float extraZ; // extra Z
        private float u; // U (texture U)
        private float v; // V (texture V)
        private boolean mirror; // boolean 
        private float textureWidth; // textureWidth
        private float textureHeight; // textureHeight
    ...
    /**
     * Overrides default render function and sends to dynamic render...
     * but will not render dynamically
     */
    @Override
    ...
    /**
     * Overrides default render function with RGBA and sends to dynamic render...
     * but will not render dynamically
     */
    @Override
    ...
    /**
     * render cuboids dynamic using VertexConsumerProvider by calling next function with vertexConsumer from spriteId
     * @param ticked
     * @param tick
     * @param matrices
     * @param vertexConsumers
    ...
    public ObjectList<DynamicPart[]> getSeeds() {
        ...
        seeds.addAll(getCuboids().stream().map(cuboid -> cuboid.parts).collect(Collectors.toList()));
        return seeds;
    }
    /**
     ...
     * @return
     */
	public static ObjectList<DynamicModelPart> generateModelParts(DynamicModel dynamicModel, float[][] x, float[][] y, float[][] z, int[][] sizeX, int[][] sizeY, int[][] sizeZ, float[][] extra, int[][] u, int[][]v, float[][] rotation, SpriteIdentifier[] sprite, Function<Identifier, RenderLayer> layerFactory) {		
		ObjectList<DynamicModelPart> MODEL_PARTS = new ObjectArrayList<DynamicModelPart>();
        for(int i = 0; i < x.length; i++) {
            MODEL_PARTS.add(generateModelPart(dynamicModel, u[i], v[i], x[i], y[i], z[i], sizeX[i], sizeY[i], sizeZ[i], extra[i], rotation[i], defaultSeeds(x.length), sprite[i], layerFactory));
        }
		return MODEL_PARTS;
    }
    /**
     ...
     * @param spriteId -- SpriteIdentifier for texture
     * @param layerFactory -- RenderLayer Factory Function
     * @return DyanmicModelPart
     */
    public static DynamicModelPart generateModelPart(DynamicModel dynamicModel, int[] u,int[] v,float[] x,float[] y,float[] z,int[] sizeX,int[] sizeY,int[] sizeZ, float[] extra, float[] rotation, ObjectList<DynamicPart[]> seeds, SpriteIdentifier spriteId, Function<Identifier,RenderLayer> layerFactory) {
        return new DynamicModelPart(dynamicModel, x, y, z, sizeX, sizeY, sizeZ, extra, u, v, rotation, seeds, spriteId, layerFactory);
    }

    /**
     * generates a DynamicModelPart given the same data as the previous function but all (u, v, x, y, z, sizeX, sizeY, sizeZ, extra) are placed in one array
     * @param dynamicModel -- the parent DynamicModel
     ...
     * @return DynamicModelPart
     */
    public static DynamicModelPart generateModelPart(DynamicModel dynamicModel, float[] allCuboids, float[] rotation, SpriteIdentifier spriteId, ObjectList<DynamicPart[]> seeds, Function<Identifier, RenderLayer> layerFactory) {
        ...
        return generateModelPart(dynamicModel, u, v, x, y, z, sizeX, sizeY, sizeZ, extra, rotation, seeds, spriteId, layerFactory);
    }
    ...
    public static ObjectList<DynamicPart[]> defaultSeeds(int numberOfCuboids) {
        ...
        for(int index = 0; index < numberOfCuboids; index++) {
            ...
            for(int dEnumIndex = 0; dEnumIndex < DynamicModelPart.DYNAMIC_ENUM_LENGTH; dEnumIndex++) {
                ...
                (DEFAULT_STATE.length-1 < dEnumIndex) ? DEFAULT_STATE[DEFAULT_STATE.length-1] : DEFAULT_STATE[dEnumIndex], 
                (DEFAULT_MIN.length-1 < dEnumIndex) ? DEFAULT_MIN[DEFAULT_MIN.length-1] : DEFAULT_MIN[dEnumIndex], 
                (DEFAULT_MAX.length-1 < dEnumIndex) ? DEFAULT_MAX[DEFAULT_MAX.length-1] : DEFAULT_MAX[dEnumIndex], 
                0F, // value
                (DEFAULT_LERP_PERCENT.length-1 < dEnumIndex) ? DEFAULT_LERP_PERCENT[DEFAULT_LERP_PERCENT.length-1] : DEFAULT_LERP_PERCENT[dEnumIndex], 
                (DEFAULT_APPLY_RANDOM_MAX.length-1 < dEnumIndex) ? DEFAULT_APPLY_RANDOM_MAX[DEFAULT_APPLY_RANDOM_MAX.length-1] : DEFAULT_APPLY_RANDOM_MAX[dEnumIndex], 
                (DEFAULT_APPLY_RANDOM_MIN.length-1 < dEnumIndex) ? DEFAULT_APPLY_RANDOM_MIN[DEFAULT_APPLY_RANDOM_MIN.length-1] : DEFAULT_APPLY_RANDOM_MIN[dEnumIndex], 
                (DEFAULT_APPLY_RANDOM_MULTIPLIER.length-1 < dEnumIndex) ? DEFAULT_APPLY_RANDOM_MULTIPLIER[DEFAULT_APPLY_RANDOM_MULTIPLIER.length-1] : DEFAULT_APPLY_RANDOM_MULTIPLIER[dEnumIndex]);
